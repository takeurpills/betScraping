...
filter_button = driver.find_element_by_xpath(
    '//div[contains(@data-atid, "time-filter")]'
)
filter_button.click()
time.sleep(LOAD_PAUSE_TIME)

live_checkbox = driver.find_element_by_xpath(
    '//span[contains(@class, "content-filter") and contains(text(), "Live")]'
)
live_checkbox.click()
time.sleep(LOAD_PAUSE_TIME)

scroll_context_selector = driver.find_element_by_xpath(
    '//a[@title="Zápas"][@class="active"]'
)
...
while len(end_of_page) < 1:
    ...
    end_of_page = driver.find_elements_by_xpath(
        "//*[contains(text(), 'Prejsť na mobilnú verziu')]"
    )
...
for league in leagues:
    if (
        league.find_element_by_xpath(
            './/div[@class="content-box-bets-subtitle no-wrap bet"]'
        ).text
        == "ZÁPAS"
    ):  # select only 1x2 type matches
        matches_parent = league.find_element_by_xpath("../../..")
        matches = league.find_elements_by_xpath(
            './/div[@class="flex bet-view-prematch-row"]'
        )
        ...
        for match in matches:
            ...
            match_odds_parents = match.find_elements_by_xpath(
                './/*[contains(@class, "bet-box-simple")]'
            )
            match_odds_list = []
            odds = ""
            for match_odds in match_odds_parents:
                odds = match_odds.find_elements_by_xpath(
